...
function renderUpcoming(){
  upcomingDiv.innerHTML = `
    <div class="featured-album">
      <img src="https://github.com/CX-Banger/cx-muzik/blob/main/media/sorties/avenir0.jpg?raw=true" class="featured-album-cover" alt="Avenir">
...
if(artistPlayBtn) {
  artistPlayBtn.addEventListener('click', ()=>{
    if(currentArtistData && currentArtistData.tracks.length > 0){
      const firstTrack = currentArtistData.tracks[0];
      playlist = currentArtistData.tracks.map(x=>({src: Array.isArray(x.src)? x.src[0] : x.src, title: x.title, artist: currentArtistData.name, thumb: x.cover || currentArtistData.photo}));
      currentIndex = 0;
      loadAndPlay(currentIndex);
    }
  });
